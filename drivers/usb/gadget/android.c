...
#define WIFI_MAC_ADDR		"/data/simcom/macAddr/mac_addr.file"
#define BT_MAC_ADDR		"/data/simcom/btadd/bt_add.file"
//[2011/09/28 simt-leijun]}
...
#ifdef CONFIG_PONYO_DOCOMO_USB_PID_VID
void android_set_vendor_id(__u16 vid)
{
...
#ifdef CONFIG_PONYO_DOCOMO_USB_PID_VID
                if(usb_testmode)
			android_set_default_product(0X902C);	
		else
			android_set_default_product(0x21F7);	
#else
		for (index = 0; index < dev->num_products; index++, up++) {
...
//[2011/09/28 simt-leijun]{
int get_usb_string(char *buf1, char *buf2)
{
	struct   file   *filp;
	mm_segment_t   old_fs;  
	loff_t pos = 0;
	...
	filp  = filp_open(WIFI_MAC_ADDR, O_RDONLY, 0777);
	if(IS_ERR(filp) ){
	...
	old_fs = get_fs();   
	set_fs(KERNEL_DS);
	filp->f_op->read(filp, tmp_buf, 30, &pos);   
	strcpy(buf1,  tmp_buf);
	...
	filp = filp_open(BT_MAC_ADDR, O_RDONLY, 0777);
	if(IS_ERR(filp)) {
		printk("error occured while opening file %s\n",  BT_MAC_ADDR);
		return -1;
	}
