...
struct msm_nand_chip {
	...
	unsigned MODEM_CFG0,MODEM_CFG1;
	uint32_t modem_ecc_buf_cfg;
	//Add end
};
...
/*Add by hxh for modem partition op,2011-07-23*/
static int
msm_nand_modem_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	struct msm_nand_chip *chip = mtd->priv;
	struct {
		dmov_s cmd[8 * 7 + 2];
	...
	while (page_count-- > 0) {
		...
		if (ops->mode != MTD_OOB_RAW) {
			dma_buffer->data.cfg0 = chip->MODEM_CFG0;
			dma_buffer->data.cfg1 = chip->MODEM_CFG1;
			if (enable_bch_ecc)
				dma_buffer->data.eccbchcfg = chip->ecc_bch_cfg;
		} else {
			dma_buffer->data.cfg0 = (chip->CFG0_RAW &
					~(7U << 6)) | ((cwperpage-1) << 6);
			dma_buffer->data.cfg1 = chip->CFG1_RAW |
						(chip->MODEM_CFG1 & CFG1_WIDE_FLASH);
		}
		...
		for (n = 0; n < cwperpage ; n++) {
			...
			if (n == 0) {
				...
				dma_buffer->data.ecccfg = chip->modem_ecc_buf_cfg;
				cmd->cmd = 0;
				cmd->src = msm_virt_to_dma(chip,
						 &dma_buffer->data.ecccfg);
				cmd->dst = MSM_NAND_EBI2_ECC_BUF_CFG;
			...
			if (ops->mode != MTD_OOB_RAW)
				...
				/*sectordatawritesize = (n < (cwperpage - 1)) ? //zys_FOTA
					516 : (512 - ((cwperpage - 1) << 2));*/
				/*	sectordatawritesize = (n < (cwperpage - 1)) ?
					512 : (512 - ((cwperpage - 1) << 2));*/
			else
				sectordatawritesize = 528;
...
err_dma_map_oobbuf_failed:
	if (ops->datbuf)
		dma_unmap_page(chip->dev, data_dma_addr, ops->len,
				DMA_TO_DEVICE);
	if (err)
		pr_err("msm_nand_modem_write_oob %llx %x %x failed %d\n",
		       to, ops->len, ops->ooblen, err);
...
static int
msm_nand_modem_write_oob_dualnandc(struct mtd_info *mtd, loff_t to,
				struct mtd_oob_ops *ops)
{
	struct msm_nand_chip *chip = mtd->priv;
	struct {
	...
	unsigned cwperpage;
		...
		pr_info("%s:\nto 0x%llx mode %d\ndatbuf 0x%p datlen 0x%x"
				...
				__func__, to, ops->mode, ops->datbuf, ops->len,
				ops->oobbuf, ops->ooblen);
		pr_info("msm_nand_modem_write_oob_dualnandc partition name %s, index %d", mtd->name,mtd->index);
#endif
	...
	if (ops->oobbuf) {
		oob_dma_addr_curr = oob_dma_addr =
			msm_nand_dma_map(chip->dev, ops->oobbuf,
				       ops->ooblen, DMA_TO_DEVICE);
		if (dma_mapping_error(chip->dev, oob_dma_addr)) {
			pr_err("msm_nand_modem_write_oob_dualnandc:"
				"failed to get dma addr "
			       "for %p\n", ops->oobbuf);
			err = -EIO;
			goto err_dma_map_oobbuf_failed;
	...
	while (page_count-- > 0) {
		...
		if (ops->mode != MTD_OOB_RAW) {
			dma_buffer->data.cfg0 = ((chip->MODEM_CFG0 & ~(7U << 6))
				& ~(1 << 4)) | ((((cwperpage >> 1)-1)) << 6);
			dma_buffer->data.cfg1 = chip->MODEM_CFG1;
		} else {
			dma_buffer->data.cfg0 = ((chip->CFG0_RAW &
...
static int msm_nand_modem_read_oob(struct mtd_info *mtd, loff_t from,
			     struct mtd_oob_ops *ops)
{
	struct msm_nand_chip *chip = mtd->priv;
	...
	if (ops->oobbuf) {
		...
		oob_dma_addr_curr = oob_dma_addr =
			msm_nand_dma_map(chip->dev, ops->oobbuf,
				       ops->ooblen, DMA_BIDIRECTIONAL);
		if (dma_mapping_error(chip->dev, oob_dma_addr)) {
			pr_err("msm_nand_modem_read_oob: failed to get dma addr "
			       "for %p\n", ops->oobbuf);
			err = -EIO;
			goto err_dma_map_oobbuf_failed;
		}
	...
	while (page_count-- > 0) {
		...
		for (n = start_sector; n < cwperpage; n++) {
			...
			if (ops->datbuf) {
				if (ops->mode != MTD_OOB_RAW)
					sectordatasize = 512;
					/*sectordatasize = (n < (cwperpage - 1)) //zys_FOTA
					? 516 : (512 - ((cwperpage - 1) << 2));*/
					/*sectordatasize = (n < (cwperpage - 1))
					? 512 : (512 - ((cwperpage - 1) << 2));*/
				else
...
#if VERBOSE
		if (rawerr && !pageerr) {
			pr_err("msm_nand_modem_read_oob %llx %x %x empty page\n",
			       (loff_t)page * mtd->writesize, ops->len,
			       ops->ooblen);
		} else {
			for (n = start_sector; n < cwperpage; n++)
...
err_dma_map_oobbuf_failed:
	...
	ops->oobretlen = ops->ooblen - oob_len;
	if (err)
		pr_err("msm_nand_modem_read_oob %llx %x %x failed %d, corrected %d\n",
		       from, ops->datbuf ? ops->len : 0, ops->ooblen, err,
		       total_ecc_errors);
#if VERBOSE
	pr_info("\n%s: ret %d, retlen %d oobretlen %d\n",
...
static int msm_nand_modem_read_oob_dualnandc(struct mtd_info *mtd, loff_t from,
			struct mtd_oob_ops *ops)
{
	struct msm_nand_chip *chip = mtd->priv;
	...
	unsigned cwperpage;
		...
		pr_info("%s:\nfrom 0x%llx mode %d\ndatbuf 0x%p datlen 0x%x"
				...
				__func__, from, ops->mode, ops->datbuf,
				ops->len, ops->oobbuf, ops->ooblen);
       	pr_info("msm_nand_modem_read_oob_dualnandc partition name %s, index %d, interleave_enable %d", mtd->name,mtd->index,interleave_enable);
#endif
	...
	if (ops->mode != MTD_OOB_RAW) {
		if (ops->datbuf != NULL && (ops->len % mtd->writesize) != 0) {
			pr_err("%s: unsupported ops->len, %d, writesize %d\n",
	...
	if (ops->datbuf) {
		data_dma_addr_curr = data_dma_addr =
			msm_nand_dma_map(chip->dev, ops->datbuf, ops->len,
				       DMA_FROM_DEVICE);
		if (dma_mapping_error(chip->dev, data_dma_addr)) {
			pr_err("msm_nand_modem_read_oob_dualnandc: "
				"failed to get dma addr for %p\n",
				ops->datbuf);
			return -EIO;
		}
	...
	oob_col = start_sector * 0x210;
	if (chip->MODEM_CFG1 & CFG1_WIDE_FLASH)
		oob_col >>= 1;
...
static int
msm_nand_modem_read(struct mtd_info *mtd, loff_t from, size_t len,
	      size_t *retlen, u_char *buf)
{
	int ret;
	struct mtd_oob_ops ops;
...
#if VERBOSE
	pr_info("msm_nand_modem_read %llx %x\n", from, len); 
#endif
	...
	ops.oobbuf = NULL;
	if (!dual_nand_ctlr_present)
		ret =  msm_nand_modem_read_oob(mtd, from, &ops);
	else
		ret = msm_nand_modem_read_oob_dualnandc(mtd, from, &ops);
	*retlen = ops.retlen;
	return ret;
}
...
static int msm_nand_modem_write(struct mtd_info *mtd, loff_t to, size_t len,
			  size_t *retlen, const u_char *buf)
{
	int ret;
	struct mtd_oob_ops ops;
	...
	ops.oobbuf = NULL;
	if (!dual_nand_ctlr_present)
		ret =  msm_nand_modem_write_oob(mtd, to, &ops);
	else
		ret =  msm_nand_modem_write_oob_dualnandc(mtd, to, &ops);
	*retlen = ops.retlen;
	return ret;
}

static int
msm_nand_modem_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	int err;
	struct msm_nand_chip *chip = mtd->priv;
	struct {
	...
	unsigned page = 0;
       ...
       #if VERBOSE 
       pr_info("msm_nand_modem_erase partition name %s, index %d", mtd->name,mtd->index);
	printk("msm_nand_modem_erase partition name %s, index %d", mtd->name,mtd->index);   
	#endif
	if (mtd->writesize == 2048)
		page = instr->addr >> 11;
	...
	dma_buffer->data.exec = 1;
	dma_buffer->data.flash_status = 0xeeeeeeee;
	dma_buffer->data.cfg0 = chip->MODEM_CFG0 & (~(7 << 6));  /* CW_PER_PAGE = 0 */
	dma_buffer->data.cfg1 = chip->MODEM_CFG1;
	dma_buffer->data.clrfstatus = 0x00000020;
	dma_buffer->data.clrrstatus = 0x000000C0;
...
static int
msm_nand_modem_erase_dualnandc(struct mtd_info *mtd, struct erase_info *instr)
{
	int err;
	struct msm_nand_chip *chip = mtd->priv;
	struct {
	...
	unsigned page = 0;
	#if VERBOSE 
       pr_info("msm_nand_modem_erase_dualnandc partition name %s, index %d", mtd->name,mtd->index);
	printk("msm_nand_modem_erase partition name %s, index %d", mtd->name,mtd->index);    
	#endif
	if (mtd->writesize == 2048)
		page = instr->addr >> 11;
...
static int
msm_nand_modem_block_isbad(struct mtd_info *mtd, loff_t ofs)
{
	struct msm_nand_chip *chip = mtd->priv;
	int ret;
	struct {
...
static int
msm_nand_modem_block_isbad_dualnandc(struct mtd_info *mtd, loff_t ofs)
{
	struct msm_nand_chip *chip = mtd->priv;
	int ret;
	struct {
...
static int
msm_nand_modem_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	struct mtd_oob_ops ops;
	int ret;
	uint8_t *buf;
	...
	*/
	...
	ops.oobbuf = NULL;
	if (!interleave_enable)
		ret =  msm_nand_modem_write_oob(mtd, ofs, &ops);
	else
		ret = msm_nand_modem_write_oob_dualnandc(mtd, ofs, &ops);
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
...
#if 1
	//Add by hxh for modem partition op,2011-07-23
	chip->MODEM_CFG0 = (((mtd_writesize >> 9)-1) << 6) /* 4/8 cw/pg for 2/4k */
	...
	chip->MODEM_CFG1 = (0 <<  0)  /* Enable ecc */
		|    (7 <<  2)  /* 8 recovery cycles */
		|    (0 <<  5)  /* Allow CS deassertion */
		/* Bad block marker location */
		|  ((mtd_writesize - ((enable_bch_ecc ? (wide_bus ? 532 : 531)
		: 528) * ((mtd_writesize >> 9) - 1)) + 1)  <<  6)
		|    (0 << 16)  /* Bad block in user data area */
		|    (30 << 17)  /* 6 cycle tWB/tRB */ //zys_timeout_error (2 << 17)
		| ((wide_bus) ? CFG1_WIDE_FLASH : 0); /* Wide flash bit */
	
#else

	chip->MODEM_CFG0 = chip->CFG0;
	chip->MODEM_CFG1 = chip->CFG1;

#endif
	chip->modem_ecc_buf_cfg = 0x1FF;
	//Add end
	...
	if (enable_bch_ecc) {
		...
		//Add by hxh for modem partition op,2011-07-23
		chip->MODEM_CFG1 |=  (1 << 27); /* Enable BCH engine */
		//Add end
	}

	pr_info("CFG0 Init  : 0x%08x\n", chip->CFG0);
	pr_info("CFG1 Init  : 0x%08x\n", chip->CFG1);
	pr_info("MODEM_CFG0 Init  : 0x%08x\n", chip->MODEM_CFG0);
	pr_info("MODEM_CFG1 Init  : 0x%08x\n", chip->MODEM_CFG1);
	...
	//Add by hxh for modem partition op,2011-07-23
	mtd->modem_erase = msm_nand_modem_erase;
	mtd->modem_block_isbad = msm_nand_modem_block_isbad;
	mtd->modem_block_markbad = msm_nand_modem_block_markbad;
	mtd->modem_read = msm_nand_modem_read;
	mtd->modem_write = msm_nand_modem_write;
	mtd->modem_read_oob  = msm_nand_modem_read_oob;
	mtd->modem_write_oob = msm_nand_modem_write_oob;
	//Add end
	if (dual_nand_ctlr_present) {
		mtd->read_oob = msm_nand_read_oob_dualnandc;
		mtd->write_oob = msm_nand_write_oob_dualnandc;
		//Add by hxh for modem partition op,2011-07-23
		mtd->modem_read_oob = msm_nand_modem_read_oob_dualnandc;
		mtd->modem_write_oob = msm_nand_modem_write_oob_dualnandc;
		//Add end
		if (interleave_enable) {
			mtd->erase = msm_nand_erase_dualnandc;
			mtd->block_isbad = msm_nand_block_isbad_dualnandc;
			//Add by hxh for modem partition op,2011-07-23
			mtd->modem_erase = msm_nand_modem_erase_dualnandc;
			mtd->modem_block_isbad = msm_nand_modem_block_isbad_dualnandc;
			//Add end
		}
	}
