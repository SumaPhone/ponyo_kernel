...
#ifdef SIMCUST_SMD
#define POWER_ON_NOMAL 0
#define POWER_OFF_CHARGE 1
#define RESET_FROM_PWROFF_CHG 1005
...
#ifdef SIMCUST_SMD
static int not_reset_from_pwroffchg(void)
{
    int ret;
    unsigned reason = 0;
    smsm_get_reset_reason(&reason);
    ret = (RESET_FROM_PWROFF_CHG ==  reason) ? 0 : 1 ;
    return ret;
}

static int rmt_storage_probe_skip(struct platform_device *pdev)
{
    if( is_emmc_kernel() 
      && (POWER_OFF_CHARGE == battchg_pause_kernel) 
      && not_reset_from_pwroffchg())
    {
        pr_err("%s: skip for power off charging on emmc fs \n", __func__);
        g_pdev = pdev;
        return -ENXIO;/*SWH*/
    }
    else
    {
        pr_err("%s: nomal init without skip \n", __func__);
        return rmt_storage_probe(pdev);
    }
}

extern void rmt_storage_probe_delay(void)
{
    pr_err("%s: run rmt_storage_probe() after exiting power off charging \n", __func__);
    rmt_storage_probe(g_pdev);
}
EXPORT_SYMBOL(rmt_storage_probe_delay);
...
static struct rmt_storage_srv msm_srv = {
	...
	.plat_drv = {
#ifdef SIMCUST_SMD
		.probe	  = rmt_storage_probe_skip, // rmt_storage_probe,/*SWH*/
#else
