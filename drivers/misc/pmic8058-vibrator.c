...
//[SIMT-lilening-20110804] add vibrator start voltage
#define VIB_START_VOLTAGE_mV 1200
...
#endif
//[SIMT-lilening-20110804] add vibrator start voltage{
static ssize_t pmic8058_vib_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long vib_level = simple_strtoul(buf, NULL, 10);
	struct pmic8058_vib *vib = dev_get_drvdata(dev);
	u8 val;
	int rc;
	...
	}else
	{
		...
		if(vib_level >3300 || vib_level<2500)
		{
			printk("invalid level\n");
			return -1;
		}
		val = vib->reg_vib_drv;
		val |= ((vib_level << VIB_DRV_SEL_SHIFT) & VIB_DRV_SEL_MASK);
		rc = pmic8058_vib_write_u8(vib, val, VIB_DRV);
		if (rc < 0)
			return rc;
		vib->reg_vib_drv = vib_level;
	}
	return 0;
}
static DEVICE_ATTR(vib_set, 0644, NULL, pmic8058_vib_store);
//[SIMT-lilening-20110804]}
