...
/* + qiukj add for getting original data from memsicd*/
int OriginalVal[16]; 
/* - qiukj add for getting original data from memsicd*/

// [SIMT-qiukejun-110727]{ used to detect version for ponoy
extern int detect_version(int *gpio_value_array);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case ECOMPASS_IOC_SET_YPR:
		if (copy_from_user(ypr, pa, sizeof(ypr)))
			return -EFAULT;
		/* + qiukj add for getting original data from memsicd*/
		//memcpy(OriginalVal,ypr,ARRAYSIZE(OriginalVal)); 
		if (copy_from_user(OriginalVal, pa, sizeof(OriginalVal)))
			return -EFAULT;
		//printk("[%s] memcpy ok\r\n",__func__);
		/* - qiukj add for getting original data from memsicd*/
		...
		if (atomic_read(&o_flag)) {
			//printk("[%s]qiukj atomic_read(&o_flag)%d %d %d %d\r\n",__func__,ypr[8],ypr[9],ypr[10],ypr[11]);// qiukj add for test
		...
		break;
/* + qiukj add for app can get original data of orientation*/
	case ECOMPASS_IOC_GET_YPR:
		if(arg)
		{
			if (copy_to_user(pa,OriginalVal,sizeof(OriginalVal)))
				return -EFAULT;
			#if 0
			{
				unsigned char i=0;
				for(;i<16;i++)
					printk("[%s] ECOMPASS_IOC_GET_YPR ok OriginalVal[%d]= %d\r\n",__func__,i,OriginalVal[i]);
			}	
			#endif
		}
		break;
/* - qiukj add for app can get original data of orientation*/
// [SIMT-qiukejun-110727]{ used to detect version for ponoy
    case IOC_GET_GPIO_VALUE:
	if(arg)
	{
		int gpio_values[2]={0};
		detect_version(gpio_values);
		if (copy_to_user(pa,gpio_values,sizeof(gpio_values)))
			return -EFAULT;
	}
	break;
// [SIMT-qiukejun-110727]}
