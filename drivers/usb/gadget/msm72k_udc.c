...
//[simt-leijun-110831]{
#define	CHARGE_NAME		"MSM_CHARGE"
static int CHARGE_OR_UDC = 0;
//[simt-leijun-110831]}
...
extern char usb_testmode;
extern void testmode_init(void);
extern int testmode_set(char usb_mode);
extern char testmode_get(void);
#endif
//chenping add for ponyo pidvid 20110721
...
static int usbcharg_flag=0;

extern int battchg_pause_kernel;

static int detect_the_usb_state(struct usb_info *ui)
{
	 //printk("...........USB_PORTSC is 0x%08x \n",readl(USB_PORTSC));
	...
	{
	     //usb not on line ,should power off 
               usbcharg_flag=1;
	}
        if (((readl(USB_PORTSC) & PORTSC_CCS) == PORTSC_CCS)) 
	{
	      // usb on line
              usbcharg_flag=0;
	}
	else
	{
	 ...
	 printk("%s,usbcharg_flag= %d\n", __func__,usbcharg_flag);
	
	return 0;
}

void checking_power_on_reason(void)
{
        printk("%s,battchg_pause_kernel is %d  \n",__func__,battchg_pause_kernel);
        if(battchg_pause_kernel)
        {
		    detect_the_usb_state(the_usb_info);
        }
}
EXPORT_SYMBOL(checking_power_on_reason);
//[simt-guliangzeng-110913]}
...
//[simt-leijun-110831]{
void set_udc_switch_state(int state)
{
	struct usb_info *ui = the_usb_info;

	CHARGE_OR_UDC = 1;        
	switch_set_state(&ui->sdev, state);
	CHARGE_OR_UDC = 0;        

	return;
}
EXPORT_SYMBOL(set_udc_switch_state);
//[simt-leijun-110831]}
	...
	for (;;) {
		...
		case USB_STATE_ONLINE:
			...
			if (flags & USB_FLAG_CONFIGURED) {
				 ...
				 * accordingly.
				 */
		//chenping add for usb_if test 20110806
		#ifdef CONFIG_PONYO_DOCOMO_USB_PID_VID
		               if(atomic_read(&ui->configured))
...
{
        int rc;
        char testmode=buf[0] - 0x30;
        if(testmode !=0 && testmode!=1)
        {
                //printk("testmode_store,testmode is %d\n",testmode);
		if(buf[0]=='I' && buf[1]=='N' &&buf[2]=='I'&&buf[3]=='T')
		{
		    // printk("testmode_store,testmode init\n");
		     testmode_init();
		     return count;
		}
		else
        ...
        rc = testmode_set(testmode);
        if(rc!=0)
        {
              printk("testmode_set failed\n");
	      return -1;
        }
	return count;
}

static ssize_t usb_testmode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	size_t count = 1;
        char testmode=0;
...
//chenping add for ponyo pidvid 20110721
#ifdef CONFIG_PONYO_DOCOMO_USB_PID_VID
static DEVICE_ATTR(usb_testmode, S_IWUSR | S_IRUSR, usb_testmode_show, usb_testmode_store);
#endif
//chenping add for ponyo pidvid 20110721
...
//[simt-20110913 guliangzeng]{
static int usbcharg_read_proc(char *page, char **start, off_t offset,
                                        int count, int *eof, void *data)
{
        *eof = 1;
        return sprintf(page, "%d", usbcharg_flag);
}
//[simt-20110913 guliangzeng]{
...
int usb_gadget_register_driver(struct usb_gadget_driver *driver)
{
	...
	int			retval, n;
  //[simt-20110913 guliangzeng]{
        struct proc_dir_entry *usbcharg_dir;  
  //[simt-20110913 guliangzeng]}
...
//chenping add for ponyo pidvid 20110721
 #ifdef CONFIG_PONYO_DOCOMO_USB_PID_VID
        retval = device_create_file(&ui->gadget.dev, &dev_attr_usb_testmode);
	if (retval != 0)
		dev_err(&ui->pdev->dev, "failed to create sysfs entry:"
			"(usb_testmode) error: (%d)\n", retval);
#endif
//chenping add for ponyo pidvid 20110721
	...
	usb_start(ui);
       ...
       //[simt-20110913 guliangzeng]{
        usbcharg_dir = proc_mkdir("usbcharg", NULL);
          if (usbcharg_dir == NULL) {
                  pr_err("Unable to create /proc/usbcharg directory");
                  retval=-ENOMEM;
                  goto fail;
          }
          /* read only proc entries */
          if (create_proc_read_entry("ischarging", 0, usbcharg_dir,
                                  usbcharg_read_proc, NULL) == NULL) {
                  pr_err("Unable to create /proc/usbcharg/ischarging entry");
                  retval = -ENOMEM;
                  goto procfail;
          }
	return 0; 
...
procfail:
    remove_proc_entry("ischarging",usbcharg_dir);
	  //[simt-20110913 guliangzeng]}
fail:
	ui->driver = NULL;
