...
static int msm_hsusb_init_rpc_ids(unsigned long vers)
{
	if (vers == 0x00010001) {
		...
		#ifdef CONFIG_PONYO_DOCOMO_USB_PID_VID
		usb_rpc_ids.usb_get_nv_imei = 99;
		#endif
		return 0;
...
#ifdef  CONFIG_PONYO_DOCOMO_USB_PID_VID
int msm_hsusb_get_nv_imei(char *imei)
{
	int rc = 0,count=0;
	struct hsusb_start_req {
		struct rpc_request_hdr hdr;
	...
	rc = msm_rpc_call_reply(usb_ep, usb_rpc_ids.usb_get_nv_imei,
				&req, sizeof(req),
				&rep, sizeof(rep), 5 * HZ);
	...
	if (rc < 0) {
		pr_err("%s: rpc call failed! error: (%d)"
				"proc id: (%lx)\n",
				__func__, rc,
				usb_rpc_ids.usb_get_nv_imei);
		return rc;
	}
	for(count=0;count<17;count++)
	{
	        //printk("///%d:value:0x%x\n",count,rep.nv_imei[count]);
                imei[count] = (rep.nv_imei[count] &0x000000ff)?(rep.nv_imei[count]):((rep.nv_imei[count])>>24);
	}
	//pr_err("%s: ////get nv imei ok: (%s)\n", __func__, imei);
	rc = 0;
	return rc;
}
EXPORT_SYMBOL(msm_hsusb_get_nv_imei);
#endif
