...
//[simt-leijun-110831]{
static int WALLCHARGE_SDCHARGE = 0;
enum {
	WALLCHARGE_UNPLUG = 0,
	WALLCHARGE_PLUG = 1,
};
...
extern void checking_power_on_reason(void);
//[simt-guliangzeng-110913]}
extern void set_udc_switch_state(int state);

//[simt-leijun-110927]{
extern void set_sdp_switch_state(int state);
//[simt-leijun-110927]}
...
//[simt-leijun-110831]{
	...
	{
		//printk("charger = USB_CHG_TYPE__WALLCHARGER***%s\n", __func__);
		WALLCHARGE_SDCHARGE = 1;
		set_udc_switch_state(WALLCHARGE_PLUG);
	}
	else if((new_chg == USB_CHG_TYPE__INVALID) && WALLCHARGE_SDCHARGE)
	{
		//printk("unplug*charger = %d***%s\n", new_chg, __func__);
		WALLCHARGE_SDCHARGE = 0;
		set_udc_switch_state(WALLCHARGE_UNPLUG);
	}
//[simt-leijun-110831]}

//[simt-leijun-110927]{
	if ((new_chg == USB_CHG_TYPE__SDP) && (!SDPCHARGE))
	{
		printk("charger = USB_CHG_TYPE__SDPCHARGER***%s\n", __func__);
		SDPCHARGE = 1;
		set_sdp_switch_state(SDPCHARGE_PLUG);
	}
	else if((new_chg == USB_CHG_TYPE__INVALID) && SDPCHARGE)
	{
		printk("unplug*charger = %d***%s\n", new_chg, __func__);
		SDPCHARGE = 0;
		set_sdp_switch_state(SDPCHARGE_UNPLUG);
	}
//[simt-leijun-110927]}
	...
	printk("Charging with %dmA current\n", charge);
...
/*[simt-liyueyi-2011108] for keeping AP wake when charging. 
	if (new_chg == USB_CHG_TYPE__WALLCHARGER) {
		wake_lock(&dev->wlock);
		queue_work(dev->wq, &dev->sm_work);
	}
[simt-liyueyi-2011108]*/

       //[simt-guliangzeng-110913]{ 
        checking_power_on_reason();
        //[simt-guliangzeng-110913]}
