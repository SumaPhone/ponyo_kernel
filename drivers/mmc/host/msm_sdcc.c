...
#if 1
//[SIMT-yhp-20111111] add sdcard host contorl attr {
int do_not_poweron = 0;
int nopartition = 0;

int sim_sdcard_power_up(struct mmc_host *mmc)
{
	struct msmsdcc_host *host = mmc_priv(mmc);

	if(do_not_poweron){
		printk("SIM-DEBUG: not poweron, just for debug\n");
		do_not_poweron = 0;
		return 0;
	}
	if(host->plat->cfg_sdcard_power)
		host->plat->cfg_sdcard_power(mmc_dev(mmc), 1);

	msleep(30);
	return 0;
}

int sim_sdcard_power_off(struct mmc_host *mmc)
{
	struct msmsdcc_host *host = mmc_priv(mmc);

	if(host->plat->cfg_sdcard_power)
		host->plat->cfg_sdcard_power(mmc_dev(mmc), 0);
	return 0;
}

static ssize_t sdcard_redetect_store(struct device *dev,
				struct device_attribute *attr, const char *buf,size_t count)
{
	...
	if(!strncmp(buf, "remove", 6)){
		printk("SIM-DEBUG: %s:%d:remove cmd, power off sdcard and detect to remove.\n",__func__,__LINE__);
		if(host->plat->cfg_sdcard_power)
			host->plat->cfg_sdcard_power(mmc_dev(mmc), 0);

		mmc_detect_change(host->mmc, 0);

	}else if(!strncmp(buf, "install", 7)){
		printk("SIM-DEBUG: %s:%d:install cmd, power on sdcard and detect to install.\n",__func__,__LINE__);
		if(host->plat->cfg_sdcard_power)
			host->plat->cfg_sdcard_power(mmc_dev(mmc), 1);

		mmc_detect_change(host->mmc, 0);

	}else if(!strncmp(buf, "nopartition", 10)){
		printk("SIM-DEBUG: %s:%d:nopartition cmd, make the mmcblk0p1 creation failed,just for debug.\n",__func__,__LINE__);
		nopartition = 1;
	}else if(!strncmp(buf, "poweroff", 8)){
		printk("SIM-DEBUG: %s:%d:poweroff cmd, do not poweron when next time resume,just for debug\n",__func__,__LINE__);
		do_not_poweron = 1;

	}else{
		printk("SIM-DEBUG: %s:%d:wrong cmd.\n",__func__,__LINE__);
	}

	return count;

}
static DEVICE_ATTR(sdcard_redetect, 0644, NULL,sdcard_redetect_store);
//[SIMT-yhp-20111111] add sdcard host contorl attr }
#endif
...
//[SIMT-yhp-20111111] add sdcard host contorl attr {
	if(mmc->index == 2){
		int ret;
		printk("%s:add sdcard redetect sysfs, host addr: 0x%08x\n", __func__,(unsigned int)mmc);
		ret = device_create_file(&pdev->dev, &dev_attr_sdcard_redetect);
		if (ret < 0)
			printk("failed to add sdcard redetect sysfs: %d\n", ret);
	}
//[SIMT-yhp-20111111] add sdcard host contorl attr }
#endif
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
        ...
        //[SIMT-yuhaipeng-110718] Do not suspend  SDIO card without sdiowakeup_irq defined
        ...
        //[SIMT-yuhaipeng-110826]
        /*
         *  This code block seems incorrect, 
         *  1,the mmc clock is enable in above code, but it will not close if type is not SDIO
         *    or if it is SDIO without MMC_PM_WAKE_SDIO_IRQ enabled.
         *  2,It will cause about 6mA increase when sleep after Wifi opened/closed or Wifi enabled
         *  3,We don't use MMC_PM_WAKE_SDIO_IRQ, so the irq controlling code is unusable, infact
         *    we just need disable clock no matter what card type.
         *  4,This part of code in 4130 works ok, so we replace it to 4130.
         */

#if 0
		//[SIMT-yuhaipeng-110826] 4131010 original code
		if (!rc) {
			if (mmc->card && (mmc->card->type == MMC_TYPE_SDIO) &&
				(mmc->pm_flags & MMC_PM_WAKE_SDIO_IRQ)) {
				disable_irq(host->irqres->start);
...
#else
        //[SIMT-yuhaipeng-110826] 41310 code
		if (!rc) {
			/*
			...
			if (mmc->card && (mmc->card->type == MMC_TYPE_SDIO) &&
				(mmc->pm_flags & MMC_PM_WAKE_SDIO_IRQ)) {
                //[SIMT-yuhaipeng-110826] it will not run here becasue (mmc->pm_flags == 0)
				host->sdio_irq_disabled = 0;
				if (host->plat->sdiowakeup_irq) {
					enable_irq_wake(
						host->plat->sdiowakeup_irq);
...
static int msmsdcc_pm_suspend(struct device *dev)
{
	...
	//Add by Yuhaipeng 20110426,Set the sdcard_gpios when suspend/resume
	if(host->plat->cfg_sdcard_power)
		host->plat->cfg_sdcard_power(mmc_dev(mmc), 0);
...
static int msmsdcc_pm_resume(struct device *dev)
{
	...
	if(host->plat->cfg_sdcard_power){
		if(do_not_poweron){
			printk("SIM-DEBUG: %s: %d do not poweron sdcard when resume,just for test\n",__func__,__LINE__);
			do_not_poweron = 0;
		}else{
		    host->plat->cfg_sdcard_power(mmc_dev(mmc), 1);
		}
