 ...
 *
 */
#define TSIF_PKTS_IN_CHUNK_DEFAULT  (20)  /**< packets in one DM chunk */
#define TSIF_CHUNKS_IN_BUF_DEFAULT   (24)
				    ...
				    struct msm_dmov_errdata *err)
{
	...
	xfer->busy = 0;
	wake_up_interruptible(&tsif_device->wq_read);  // by yunpeng.liu
...
struct msm_tsif_device *tsif_find_by_id(int id)
{
	struct msm_tsif_device *tsif_device;
...
EXPORT_SYMBOL(tsif_find_by_id);

//[2011/09/22 simt-leijun]{
struct msm_tsif_device *get_tsif_device(void)
{
    return tsif_find_by_id(0);
}
EXPORT_SYMBOL(get_tsif_device);
//[2011/09/22 simt-leijun]}
...
//by yunpeng.liu
    init_waitqueue_head(&tsif_device->wq_read);
	tsif_device->rptr = 0;
...
void tsif_get_state(void *cookie, int *ri, int *wi, int *rptr, enum tsif_state *state)
{
	struct msm_tsif_device *tsif_device = cookie;
	if (ri)
		*ri    = tsif_device->ri;
...
ssize_t tsif_read(void *cookie,  void *buf, unsigned int count)
{
	int avail = 0;
	int wi = 0;
	struct msm_tsif_device *the_dev = cookie;
	...
	wi = (the_dev->wi > the_dev->ri) ?
		the_dev->wi : (TSIF_PKTS_IN_CHUNK_DEFAULT * TSIF_CHUNKS_IN_BUF_DEFAULT);
	avail = min(wi * TSIF_PKT_SIZE - the_dev->rptr, count);
	memcpy(buf, the_dev->data_buffer + the_dev->rptr, avail);

	the_dev->rptr = (the_dev->rptr + avail) %
		(TSIF_PKT_SIZE * (TSIF_PKTS_IN_CHUNK_DEFAULT * TSIF_CHUNKS_IN_BUF_DEFAULT));
	the_dev->ri = the_dev->rptr / TSIF_PKT_SIZE;
	tsif_reclaim_packets(the_dev, the_dev->ri);

	return 0;
}
EXPORT_SYMBOL(tsif_read);
