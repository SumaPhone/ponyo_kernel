...
static ssize_t hsd_pin_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", msm_proc_comm(PCOM_OEM_TEST_CMD, 0, 0));
}
static DEVICE_ATTR(hsd_pin, S_IRUSR,
		hsd_pin_show, NULL);

static ssize_t hsd_threshold_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
    int threshold = 200;
    int rc;
    rc = sscanf(buf, "%d", &threshold);
    msm_proc_comm(PCOM_OEM_TEST_CMD2, &threshold, &threshold);
    if (1 != rc) {
		dev_err(dev, "hsd_threshold_store rc=%d\n", rc);
    }
	return count;
}
static DEVICE_ATTR(hsd_threshold, S_IWUSR,
		NULL, hsd_threshold_store);

static ssize_t hsd_period_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
    int div;
    int time;
    int rc;
    rc = sscanf(buf, "%d %d", &div, &time);
    msm_proc_comm(PCOM_OEM_TEST_CMD3, &div, &time);
    if (2 != rc) {
		dev_err(dev, "hsd_period_store rc=%d\n", rc);
    }
	return count;
}
static DEVICE_ATTR(hsd_period, S_IWUSR,
		NULL, hsd_period_store);

static ssize_t hsd_hysteresis_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
    int div;
    ...
    int rc;
    rc = sscanf(buf, "%d %d", &div, &time);
    msm_proc_comm(PCOM_OEM_TEST_CMD4, &div, &time);
    if (2 != rc) {
		dev_err(dev, "hsd_hysteresis_store rc=%d\n", rc);
    }
	return count;
}
static DEVICE_ATTR(hsd_hysteresis, S_IWUSR,
		NULL, hsd_hysteresis_store);
...
static int __devinit hs_probe(struct platform_device *pdev)
{
	...
	rc = device_create_file(hs->sdev.dev, &dev_attr_hsd_pin);
    
	rc = device_create_file(hs->sdev.dev, &dev_attr_hsd_threshold);
	rc = device_create_file(hs->sdev.dev, &dev_attr_hsd_period);
	rc = device_create_file(hs->sdev.dev, &dev_attr_hsd_hysteresis);
