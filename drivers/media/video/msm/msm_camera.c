			...
			void __user *arg)
{
	...
	qcmd_resp = __msm_control(sync,
				  &ctrl_pmsm->ctrl_q,
				  qcmd, msecs_to_jiffies(60000)); /*yangyuan 20110921 from 10000->60000 for ponyo camera night mode capture phone reset*/
			...
			int is_controlnode)
{
	...
	if (!sync->core_powered_on && !is_controlnode) {
		...
		if (sync->vfefn.vfe_init) {
			...
			if (rc < 0) {
				pr_err("%s: sensor init failed: %d try once more !\n",
                    __func__, rc);
	    		msm_camio_sensor_clk_off(sync->pdev);
	    		msm_camvfe_fn_init(&sync->vfefn, sync);
	    		rc = msm_camio_sensor_clk_on(sync->pdev);
				...
				rc = sync->sctrl.s_init(sync->sdata);//add for open camera twice if open fail at once by lijiankun 2011-9-21
			    if (rc < 0) {
    				pr_err("%s: sensor init failed: %d once again!!\n",
    					__func__, rc);
    				msm_camio_sensor_clk_off(sync->pdev);
    				goto msm_open_err;
				}
