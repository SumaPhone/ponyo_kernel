...
static unsigned smd_reset_data = RESET_BASE;
extern uint32_t smsm_check_reboot_data(uint32_t income_reason)
{
  uint32_t ret = 0x776655AA;
  switch (smd_reset_data)
  {
    case RESET_FOR_USIM_LOSE:
		ret = 0x776655a1;
      break;
    case RESET_FOR_BATTERY_LOSE:
		ret = 0x776655a2;
      break;
    case RESET_FOR_APPS_CRASH:
		ret = 0x776655a3;
      break;
    case RESET_FOR_MODEM_CRASH:
		ret = 0x776655a4;
      break;
    case RESET_FROM_PWROFF_CHG:
		ret = 0x776655a5;
      break;
  ...
  pr_err("[SMD]%s: smd_reset_data <%d> ,income_reason<%d>, ret<%d>\n", __func__, smd_reset_data, income_reason,ret);
  return  ret;
}
EXPORT_SYMBOL(smsm_check_reboot_data);
  
void smsm_set_reset_reason(unsigned data)
{
  int ret = 0;
  unsigned data1 = 0;
  unsigned data2 = 0;
  unsigned  *pReason = NULL;
  unsigned int nLen = 2*sizeof(unsigned );

  if((data < RESET_BASE)||(data > RESET_MAX))
  {
		pr_err("[SMD]%s: Error! data out of range ! Do nothing and return directly. \n", __func__);
       return;
  }
  
  smd_reset_data = data;
    
  pReason = smem_alloc2(SMEM_ID_VENDOR0, nLen);
  if(pReason)
  {
      pr_err("[SMD]%s: data <%d> , nLen<%d>\n", __func__, data, nLen);
      pReason[0] = data;
  }
  else
  {
      pr_err("[SMD]%s: Error! smem_alloc2 return NULL ! \n", __func__);
  }

  ret = msm_proc_comm(PCOM_CUSTOMER_CMD1, &data1, &data2);
  pr_err("[SMD] %s: PCOM_CUSTOMER_CMD1 ret<%d>, d1<%d>, d2<%d>", __func__, ret, data1, data2);  
  if(ret)
  {
      pr_err("[SMD]%s: ERROR, could not PCOM_CUSTOMER_CMD1",__func__);  
  }

}
EXPORT_SYMBOL(smsm_set_reset_reason);

#define AP_BOOT_MODE1  0x77665500
#define AP_BOOT_MODE2  0x77665f00
#define AP_BOOT_MODE_OEM1  0x6f656d00
#define AP_BOOT_MODE_OEM2  0x6f656f00

#define AP_BOOT_USIM_LOSE  0x776655a1
#define AP_BOOT_BATT_LOSE  0x776655a2
#define AP_BOOT_AP_CRASH  0x776655a3
#define AP_BOOT_CP_CRASH  0x776655a4
#define AP_BOOT_FROM_CHG 0x776655a5

void smsm_get_reset_reason(unsigned *data)
{
  int ret = 0;
  unsigned data1 = 0;
  unsigned data2 = 0;
  unsigned  *pReason = NULL;
  unsigned int nLen = 2*sizeof(unsigned );
  unsigned *pPwronStatus = NULL;

  if(NULL == data)
  {
		pr_err("[SMD]%s: Error! Null point!!\n", __func__);
       return;
  }

  ret = msm_proc_comm(PCOM_CUSTOMER_CMD2, &data1, &data2);
  pr_err("[SMD] %s: PCOM_CUSTOMER_CMD2 ret<%d>, d1<%d>, d2<%d>", __func__, ret, data1, data2);  
  if(ret)
  {
     pr_err("[SMD]%s: ERROR, could not PCOM_CUSTOMER_CMD2",__func__);  
  }

  pReason = smem_find(SMEM_ID_VENDOR0, nLen);
  if(pReason)
  {
      pr_err("[SMD]%s: *pReason <%d, %d>, nLen<%d> \n", __func__, pReason[0], pReason[1], nLen);
      *data  = pReason[0];
  }
  else
  {
      pr_err("[SMD]%s: Error! get reset reason fail! \n", __func__);
  }

  pPwronStatus = smem_find(SMEM_APPS_BOOT_MODE, sizeof(unsigned));
  if(pPwronStatus)
  {
      if(((*pPwronStatus >= AP_BOOT_MODE1) && (*pPwronStatus<AP_BOOT_MODE2) )
        ||((*pPwronStatus >= AP_BOOT_MODE_OEM1) && (*pPwronStatus < AP_BOOT_MODE_OEM2)))
      {
              if( AP_BOOT_USIM_LOSE == *pPwronStatus)
              {
                *data = RESET_FOR_USIM_LOSE;   
              }
              else if( AP_BOOT_BATT_LOSE == *pPwronStatus)
              {
                *data = RESET_FOR_BATTERY_LOSE;   
              }
              else if( AP_BOOT_AP_CRASH == *pPwronStatus)
              {
                *data = RESET_FOR_APPS_CRASH;   
              }
              else if( AP_BOOT_CP_CRASH == *pPwronStatus)
              {
                *data = RESET_FOR_MODEM_CRASH;   
              }
              else if( (0x776655AA != *pPwronStatus) && ( RESET_BASE == *data  ) )  //Other reset
              {
                *data = RESET_FROM_PWROFF_CHG;   
              }

          }

/*
      if (PM_PWR_ON_EVENT_WDOG == *pPwronStatus)
      {
          *data = RESET_FOR_MODEM_CRASH;
      }
      else if (PM_PWR_ON_EVENT_SMPL == *pPwronStatus)
      {
          *data = RESET_FOR_BATTERY_LOSE;
      }
*/
  ...
  {
      pr_err("[SMD]%s: Error! get Power on status fail! \n", __func__);
  }

  pr_err("[SMD]%s: *pPwronStatus<0x%x>, *data<%d> \n", __func__, *pPwronStatus, *data);
  
}
EXPORT_SYMBOL(smsm_get_reset_reason);

void smsm_reset_chip(void)
{
    int ret = 0;
    int count = 0;
    unsigned pwrkey = 0;

    ret = msm_proc_comm(PCOM_RESET_CHIP, &pwrkey, 0);//Could it be the reset interface? y, But should never use it before set reset reason
    pr_err("[SMD] %s: PCOM_RESET_CHIP , ret<%d>, pwrkey<%d>", __func__, ret, pwrkey);  
    if(ret)
    {
        pr_err("[SMD]%s: ERROR, could not PCOM_RESET_CHIP",__func__);  
    }
    else
    {
        ...
        {
            msleep(200);
            pr_err("[SMD]%s: wait to reset chip, count<%d>",__func__, count);          
        }
    }
}
EXPORT_SYMBOL(smsm_reset_chip);
#endif
