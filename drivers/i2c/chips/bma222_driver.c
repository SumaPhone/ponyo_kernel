...
//#include "../../../../../external/kernel-headers/original/linux/i2c.h"   // qiukj add for refering I2C_CLIENT_INSMOD_1
...
#define	BMA222_OFFSET_MAX	3

//static MON_XYZ_INT 	calstep1;
//static MON_XYZ_INT 	calstep1_2;
//static MON_XYZ_INT 	calstep2;
//static signed char 	Caldata[3]={0,0,1}; // keep the mobile horizontal when calibrating
...
{
    bma222_read_accel_xyz((bma222acc_t *)&cal45A);
#ifdef BMA222_DEBUG
...
{
    bma222_read_accel_xyz((bma222acc_t *)&cal45B);
#ifdef BMA222_DEBUG
...
static int DoCal_Bosch(void)
{
	...
	calvalue.xyz.YVal = 0;
	calvalue.xyz.ZVal = 0;
	calvalue.scalex = BMA222CAL_SCALE;
	calvalue.scaley = BMA222CAL_SCALE;
	
        bma222_read_accel_xyz((bma222acc_t *)&calNumber);
#ifdef BMA222_DEBUG
	printk("DoCal_Bosch calNumber.XVal = %d ,calNumber.YVal = %d\n",calNumber.XVal,calNumber.YVal);
#endif

	if((ABS(calNumber.XVal) > BMA222CAL_MAX_OFFSET) || (ABS(calNumber.YVal) > BMA222CAL_MAX_OFFSET))
	{
#ifdef BMA222_DEBUG
...
static int bma222_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	//printk("_IOC_TYPE(cmd): %c, _IOC_NR(cmd) : %d\n",_IOC_TYPE(cmd),_IOC_NR(cmd)); // qiukj add for test
	/* check cmd */
...
*/
	...
	{
		bma222acc_t accel_xyz={0};
		bma222_read_accel_xyz((bma222acc_t*)data);
		memcpy(&accel_xyz,(bma222acc_t*)data,sizeof(accel_xyz));
		/* judge if the moblie phone is set horizontally*/
		if( BMA222_MAX_OFFSET_X <= ABS(accel_xyz.x)|| BMA222_MAX_OFFSET_Y <= ABS(accel_xyz.y))
		{
			return -EPERM;
		}
...
case BMA222_FAST_CALIBRATION:  
{
		if(copy_from_user(data,(unsigned char*)arg,3)!=0)
		{
			...
			return -EFAULT;
                 ...
                 */
                {
                  ...
                  bma222_read_accel_xyz((bma222acc_t *)&calNumber);
#ifdef BMA222_DEBUG
                  printk("BMA222_FAST_CALIBRATION calNumber.XVal = %d ,calNumber.YVal = %d\n",calNumber.XVal,calNumber.YVal);
#endif
                  
                  if((ABS(calNumber.XVal) > BMA222CAL_MAX_OFFSET) || (ABS(calNumber.YVal) > BMA222CAL_MAX_OFFSET))
                  {
#ifdef BMA222_DEBUG
                         printk("BMA222_FAST_CALIBRATION large offset, cal fail\n");
#endif
                         return -1;
                  }
                }
	...
	case MOTION_DOCAL_STEP2:
          {
#ifdef BMA222_DEBUG
            printk("driver bma222 MOTION_DOCAL_STEP2 arg %p\n", (void *)arg);
#endif
            if (!PowerState)
              err = -EBUSY;
            ...
            if (arg)
              {
                ...
                // check the data
                if ((ABS(calvalue.xyz.XVal) > BMA222CAL_MAX_OFFSET)
                    ||(ABS(calvalue.xyz.YVal) > BMA222CAL_MAX_OFFSET)
                    ||(calvalue.scalex < (BMA222CAL_SCALE - BMA222CAL_MAX_OFFSET))
                    ||(calvalue.scalex > (BMA222CAL_SCALE + BMA222CAL_MAX_OFFSET))
                    ||(calvalue.scaley < (BMA222CAL_SCALE - BMA222CAL_MAX_OFFSET))
                    ||(calvalue.scaley > (BMA222CAL_SCALE + BMA222CAL_MAX_OFFSET))
                    )
                  {
#ifdef BMA222_DEBUG
...
#ifdef BMA222_DEBUG	
            printk("driver MOTION_DOCAL_STEP2 finish\n");
#endif
            //break;
            return err;
          }
...
static struct miscdevice bma_device = {
	.minor = MOTION_MINOR, //MISC_DYNAMIC_MINOR,
	.name = "bma222",
	.fops = &bma222_fops,
};
			 ...
			 const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "%s() chip Id:0x%x i2c name:%s addr:0x%x\n",__func__,tempvalue&0xff,client->name,client->addr);
	#if 0
	{
		unsigned char i=0;
		for(;i<10*2;i++)
		{
			tempvalue=i2c_smbus_read_byte_data(client,0x0);
#ifdef BMA222_DEBUG
			printk(KERN_INFO "%s() chip Id:0x%x i2c name:%s addr:0x%x\n",__func__,tempvalue,client->name,client->addr);
#endif
