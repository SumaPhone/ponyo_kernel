...
/*******************************************************************************
 * Description: *//**\brief  This API start the fast inline calibration and store
 ...
 *                  11b ------ 0g
 * ******************************************************************************/
static int bma250_fast_calibration(signed char * data)
{
    signed char tmp;
#ifdef BMA250_DEBUG
	printk(KERN_INFO "%s\n",__FUNCTION__);
...
#endif
    bma250_set_offset_target_x(data[0]);
    //bma250_get_offset_target_x(&tmp);
...
#endif
    bma250_set_offset_target_y(data[1]);
...
#endif
    bma250_set_offset_target_z(data[2]);
    ...
    tmp=1;//unlock eeprom
    bma250_set_ee_w(tmp);
    bma250_set_ee_prog_trig(0x01);//update eeprom
    do
    {
        mdelay(2); 
...
int bma250_read_accel_xyz(bma250acc_t * acc)
    ...
    * for overflow
    */
   signed int negMaxV = -1 << 15;
   signed int maxV = 1 << 15;
   signed int cV;
   //[SIMT-huangli-110715]}
   if (p_bma250==C_Zero_U8X)
      {
      ...
      temp32 = (acc->x - calvalue.xyz.XVal);
      //[SIMT-huangli-110718]{
      /*
       * for overflow
       */
      cV = (temp32 * BMA250CAL_SCALE) /(calvalue.scalex);
      if (cV < negMaxV || cV > maxV) {
        cV = cV >> 16;
         while(cV < negMaxV || cV > maxV) {
          cV = cV >> 1;
        }
        acc->x = cV;
      } else
        acc->x = (temp32 * BMA250CAL_SCALE) /(calvalue.scalex);
      //[SIMT-huangli-110718]}
      ...
      temp32 = (acc->y - calvalue.xyz.YVal);
      //[SIMT-huangli-110718]{
      /*
       * for overflow
       */
      cV = (temp32 * BMA250CAL_SCALE) /(calvalue.scaley);
      if (cV < negMaxV || cV > maxV) {
        cV = cV >> 16;
        while(cV < negMaxV || cV > maxV) {
          cV = cV >> 1;
        ...
        acc->y = (temp32 * BMA250CAL_SCALE) /(calvalue.scaley);
      //[SIMT-huangli-110718]}
...
{
    bma250_read_accel_xyz((bma250acc_t *)&cal45A);
#ifdef BMA250_DEBUG
...
{
    bma250_read_accel_xyz((bma250acc_t *)&cal45B);
#ifdef BMA250_DEBUG
...
static int DoCal_Bosch(void)
{
	...
	calvalue.xyz.YVal = 0;
	calvalue.xyz.ZVal = 0;
	calvalue.scalex = BMA250CAL_SCALE;
	calvalue.scaley = BMA250CAL_SCALE;
	
        bma250_read_accel_xyz((bma250acc_t *)&calNumber);
#ifdef BMA250_DEBUG
	printk("DoCal_Bosch calNumber.XVal = %d ,calNumber.YVal = %d\n",calNumber.XVal,calNumber.YVal);
#endif

	if((ABS(calNumber.XVal) > BMA250CAL_MAX_OFFSET) || (ABS(calNumber.YVal) > BMA250CAL_MAX_OFFSET))
	{
#ifdef BMA250_DEBUG
	...
	case BMA250_FAST_CALIBRATION:
	{
		if(copy_from_user(data,(unsigned char*)arg,3)!=0)
		{
			...
			return -EFAULT;
                ...
                //[SIMT-huangli-110721]{
                /*
                 * horizontal calibrate failure
                 */
                {
                  MON_XYZ_INT calNumber;
                  bma250_read_accel_xyz((bma250acc_t *)&calNumber);
#ifdef BMA250_DEBUG
                  printk("BMA250_FAST_CALIBRATION calNumber.XVal = %d ,calNumber.YVal = %d\n",calNumber.XVal,calNumber.YVal);
#endif
                 
                  if((ABS(calNumber.XVal) > BMA250CAL_MAX_OFFSET) || (ABS(calNumber.YVal) > BMA250CAL_MAX_OFFSET))
                  {
#ifdef BMA250_DEBUG
                        printk("BMA250_FAST_CALIBRATION large offset, cal fail\n");
#endif
                        return -1;
                  }
                }
		...
		err = bma250_fast_calibration(data);
...
#ifdef BMA250_DEBUG
                printk("calvalue.xyz.XVal: %d\n", calvalue.xyz.XVal);
#endif
                calvalue.xyz.YVal = cal45A.YVal;
#ifdef BMA250_DEBUG
                printk("calvalue.xyz.YVal: %d\n", calvalue.xyz.YVal);
#endif
                calvalue.xyz.ZVal = 0;
                calvalue.scalex = ABS(ABS(cal45B.XVal) -ABS(cal45A.XVal));
#ifdef BMA250_DEBUG
                printk("calvalue.scalex: %d\n", calvalue.scalex);
#endif
                calvalue.scaley = ABS(ABS(cal45B.YVal) - ABS(cal45A.YVal));
#ifdef BMA250_DEBUG
                printk("calvalue.scaley: %d\n", calvalue.scaley);
#endif
	
                // check the data
                if ((ABS(calvalue.xyz.XVal) > BMA250CAL_MAX_OFFSET)
                    ||(ABS(calvalue.xyz.YVal) > BMA250CAL_MAX_OFFSET)
                    ||(calvalue.scalex < (BMA250CAL_SCALE - BMA250CAL_MAX_OFFSET))
                    ||(calvalue.scalex > (BMA250CAL_SCALE + BMA250CAL_MAX_OFFSET))
                    ||(calvalue.scaley < (BMA250CAL_SCALE - BMA250CAL_MAX_OFFSET))
                    ||(calvalue.scaley > (BMA250CAL_SCALE + BMA250CAL_MAX_OFFSET))
                    )
                  {
#ifdef BMA250_DEBUG
...
*/
	...
	{
		#if 0
		/* + qiukj add for bma250 calibration*/
		bma250acc_t accel_xyz={0};
		bma250_read_accel_xyz((bma250acc_t*)data);
		memcpy(&accel_xyz,(bma250acc_t*)data,sizeof(accel_xyz));
		/* judge if the moblie phone is set horizontally*/
		if( BMA250_MAX_OFFSET_X <= ABS(accel_xyz.x)|| BMA250_MAX_OFFSET_Y <= ABS(accel_xyz.y))
		{
			return -EPERM;
		}	
		/* - qiukj add for bma250 calibration*/
...
//[SIMT-huangli-110714]}
	...
	bma250_set_bandwidth(3);		//bandwidth 3---62.5 4---125Hz
	bma250_set_range(0);			//range 0---+/-2G   1---+/- 4G 2---+/-8G 3---+/-16G 
