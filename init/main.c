...
//sunxiaowen add begin for power down charging
#define POWER_ON_NOMAL 0
#define POWER_OFF_CHARGE 1
int battchg_pause_kernel = POWER_ON_NOMAL;
EXPORT_SYMBOL(battchg_pause_kernel);

static int boot_emmc_kernel = 0;/*SWH*/
extern int is_emmc_kernel(void)
{
    return boot_emmc_kernel;
}
EXPORT_SYMBOL(is_emmc_kernel);
    ...
    *value++ = 0;
    ...
    if(!strcmp(name,"androidboot.battchg_pause"))
    {
        if (!strcmp(value,"true"))
            battchg_pause_kernel = POWER_OFF_CHARGE;
    }

    if(!strcmp(name,"androidboot.emmc"))/*SWH*/
    {
        if (!strcmp(value,"true"))
            boot_emmc_kernel = 1;
    }
...
//sunxiaowen add end for power down charging
#endif
