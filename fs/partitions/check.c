...
#include <linux/vmalloc.h> //[SIMT-yhp-111205]
...
check_partition(struct gendisk *hd, struct block_device *bdev)
{
    ...
    //[SIMT-yhp-111205]use vmalloc instead of kzalloc
    printk("SIM-DEBUG %s: %d use vmalloc instead of kzalloc\n",__func__,__LINE__);
//	state = kzalloc(sizeof(struct parsed_partitions), GFP_KERNEL);
	state = vmalloc(sizeof(struct parsed_partitions));
	if (!state)
		return NULL;

    memset((void*)state, 0, sizeof(struct parsed_partitions));
	state->bdev = bdev;
...
extern int nopartition;//[SIMT-yhp-20111123] just for debug
				...
				sector_t start, sector_t len, int flags)
{
    ...
    //[SIMT-yhp-20111123] just for debug {
	if(nopartition){
		printk("SIM-DEBUG: %s: %d  nopartition set, just for debug\n",__func__,__LINE__);
		p = NULL;
		nopartition = 0;
	}
    //[SIMT-yhp-20111123] just for debug }
