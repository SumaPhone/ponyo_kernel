...
static int mmc328x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	unsigned char data[16] = {0};
	int res = 0;
	struct mag_platform_data *platform_data=NULL;  // qiukj add
	...
	this_client = client;
	pr_info("%s():name %s addr 0x%x\n",__func__,this_client->name,this_client->addr); // qiukj add

	/*+ qiukj add for ponyo project*/
	platform_data = client->dev.platform_data;
	if (platform_data) {
		if (platform_data->power) {
			res = platform_data->power(1);
			if (res < 0) {
				printk(KERN_ERR "mmc328x_probe power on failed\n");
				goto out;
			}
		}
	}    
