...
#include "cyttsp_extern.h"
#define DBG(x)  
#define DBG2(x) 
#define DBG3(x) 

#define DEBUG_CYTTSP 0
...
#else
#define INVERT_X(x, xmax)           (x)
#define INVERT_Y(y, ymax)           (y)
#endif
...
#define CY_SYSINFO_CALIBRATE_REG    0x02
...
//				printk("1: t->snd_trk[%d] = %d, x = %d, y = %d, w = %d, z = %d\n", id, t->snd_trk[id], t->cur_mt_pos[t->snd_trk[id]][CY_XPOS], t->cur_mt_pos[t->snd_trk[id]][CY_YPOS], t->tool_width, t->cur_mt_z[t->snd_trk[id]]);
				cyttsp_swp_xy(&t->cur_mt_pos[t->snd_trk[id]][CY_XPOS], &t->cur_mt_pos[t->snd_trk[id]][CY_YPOS]);
#if DEBUG_CYTTSP
		printk("1: t->snd_trk[%d] = %d, x = %d, y = %d, w = %d, z = %d\n", id, t->snd_trk[id], t->cur_mt_pos[t->snd_trk[id]][CY_XPOS], t->cur_mt_pos[t->snd_trk[id]][CY_YPOS], t->tool_width, t->cur_mt_z[t->snd_trk[id]]);
#endif
#if 0
				if(t->cur_mt_pos[t->snd_trk[id]][CY_YPOS] <= 900){
#else
				if(t->cur_mt_pos[t->snd_trk[id]][CY_YPOS] <= 810){
#endif
					input_report_abs(ts->input, ABS_MT_TOUCH_MAJOR,
					...
					printk("\nKEY: on\n");
#endif
					VrpCallback((TouchSampleValidFlag|TouchSampleDownFlag), t->cur_mt_pos[t->snd_trk[id]][CY_XPOS], t->cur_mt_pos[t->snd_trk[id]][CY_YPOS]);
				}
...
#if DEBUG_CYTTSP			
				printk("2: t->prv_mt_tch[%d] = %d, x = %d, y = %d, w = %d, z = %d\n", id, ts->prv_mt_tch[id], ts->prv_mt_pos[ts->prv_mt_tch[id]][CY_XPOS], ts->prv_mt_pos[ts->prv_mt_tch[id]][CY_YPOS], t->tool_width, CY_NTCH);
#endif
				/* void out this touch */
#if 0
				if(ts->prv_mt_pos[ts->prv_mt_tch[id]][CY_YPOS] <= 900){
#else
				if(ts->prv_mt_pos[ts->prv_mt_tch[id]][CY_YPOS] <= 810){
#endif
					input_report_abs(ts->input, ABS_MT_TOUCH_MAJOR,
							CY_NTCH);
					input_report_abs(ts->input, ABS_MT_WIDTH_MAJOR,
					...
					printk("\nKEY: off\n");
#endif
//					VrpCallback((TouchSampleValidFlag|TouchSampleDownFlag), ts->prv_mt_pos[ts->prv_mt_tch[id]][CY_XPOS], ts->prv_mt_pos[ts->prv_mt_tch[id]][CY_YPOS]);
					VrpCallback(TouchSampleValidFlag, 0, 0);
//					VrpCallback((TouchSampleValidFlag|TouchSampleDownFlag), 0, 0);
				}
...
//printk("3 : XXXXXXXXXXXXXXXXXXXXXXXXx\n");
...
static void cyttsp_xy_worker(struct cyttsp *ts)
{
	...
	/*check if whether the TP is in bootloader mode ? */
...
static int cyttsp_resume(struct cyttsp *ts)
{
	...
	if (ts->platform_data->use_sleep && (ts->platform_data->power_state !=
		...
		if (ts->platform_data->wakeup) {
			retval = ts->platform_data->wakeup(CYTTSP_POWER_ON);
			if (retval < 0)
...
static int cyttsp_suspend(struct cyttsp *ts)
{
	...
	DBG(printk(KERN_INFO"%s: Sleep Power state is %s\n", __func__,
		...
		"SLEEP" : "LOW POWER"));)
		if (ts->platform_data->wakeup) {
			retval = ts->platform_data->wakeup(CYTTSP_POWER_OFF);
			if (retval < 0){
				printk(KERN_ERR "%s: Error, wakeup failed!\n", __func__);
			}
		} else {
			printk(KERN_ERR "%s: Error, wakeup not implemented (check board file).\n", __func__);
			retval = -ENOSYS;
		}
	return retval;
}
...
static ssize_t firmware_write(struct file *fp, struct kobject *kobj,
...
static ssize_t firmware_read(struct file *fp, struct kobject *kobj,
...
/////
	...
	static int cyttsp_calibrate(struct cyttsp *ts, int cal){
		int retval;
		u8 intrvl_ray[3];
		...
		/* switch to Sysinfomation mode */
		retval = cyttsp_set_sysinfo_mode(ts);
		if(retval < 0){
			printk("cyttsp_calibrate ERROR !");
			return -1;
		}
		printk("Before ttsp_write_block_data\n");
		/* write data to the calibrate registers */
		intrvl_ray[0] = 0x20;
		intrvl_ray[1] = 0x00;
		intrvl_ray[2] = 0x00;
		retval = ttsp_write_block_data(ts,
				CY_SYSINFO_CALIBRATE_REG,
				sizeof(intrvl_ray), intrvl_ray);
		if(retval < 0){
			printk("cyttsp_calibrate ttsp_write_block_data ERROR!\n");
			return -2;
		}
		printk("After ttsp_write_block_data\n");
		msleep(CY_DELAY_SYSINFO);
		printk("Before cyttsp_set_operational_mode\n");
		/* switch back to Operational mode */
		retval = cyttsp_set_operational_mode(ts);
		if(retval < 0){
			printk("cyttsp_calibrate cyttsp_set_operational_mode ERROR!\n");
			return -3;
		}
		printk("After cyttsp_set_operational_mode\n");
		retval = cyttsp_power_on(ts);
		if (retval < 0)
			printk(KERN_ERR "%s: Error, power on fail\n", __func__);
		/* resume watchdog */
...
static char cbuf[16];
static ssize_t attr_calibrate_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	printk("attr_calibrate_show \n");
	return sprintf(buf, "%s",cbuf);
}

static ssize_t attr_calibrate_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	static int cal = 0;
	int ret, n;
	struct cyttsp *ts = dev_get_drvdata(dev);
	printk("attr_calibrate_store begin !\n");
	if(buf[0] != 0){
		printk("buf = %s\n", buf);
		cal = 1;
	}else{
	...
	ret = cyttsp_calibrate(ts, cal);
	if(ret < 0){
		n = sprintf(cbuf, "calibrate error !\n");
//		cbuf[n] = 0;
		//              return -EINVAL;
		return n;
	}
...
static struct device_attribute calibrate =
__ATTR(calibrate, 0666, attr_calibrate_show, attr_calibrate_store);
...
void *cyttsp_core_init(struct cyttsp_bus_ops *bus_ops, struct device *pdev)
{
	...
	if (ts->platform_data->init)
		retval = ts->platform_data->init(CYTTSP_POWER_ON);
	if (retval) {
	...
	retval = device_create_file(pdev, &calibrate);
	if (retval) {
		printk(KERN_ERR "%s: Error, could not create calibrate attribute\n",
			__func__);
		goto device_create_error;
	}
...
error_input_allocate_device:
	if (ts->platform_data->init)
		ts->platform_data->init(CYTTSP_POWER_OFF);
error_init:
	kfree(ts);
