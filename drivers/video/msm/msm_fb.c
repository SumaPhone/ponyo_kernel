...
//[SIMT-lilening-20110804] add lcd vcom valibration {
	...
	if (!vcom_inited && pdata->init_vcom)
	{
		ret = pdata->init_vcom(&g_vcom);
		if(0 == ret)
		{
...
//[SIMT-lilening-20110804] add lcd vcom valibration {
	...
	case MSMFB_SET_PANEL_VCOM:
		ret = copy_from_user(&g_vcom, argp, sizeof(g_vcom)) ;
		if (ret)
			return ret;
		if(pdata->set_vcom)
		{
			ret = pdata->set_vcom(&g_vcom);
			if (0 != ret)
				return ret;
	...
	case MSMFB_GET_PANEL_VCOM:
		if(pdata->get_vcom)
		{
			ret = pdata->get_vcom(&g_vcom);
			if (0 != ret)
				return ret;
			ret = copy_to_user(argp, &g_vcom, sizeof(g_vcom)) ;
			if (ret)
				return ret;
		}
