...
#ifdef CONFIG_USE_IRDA
static int msm_irda_register_set(void);
#endif
...
static int msm_hs_startup(struct uart_port *uport)
{
	...
	pm_runtime_enable(uport->dev);
        ...
        // wanggang add for irda 20110620 start
       #ifdef CONFIG_USE_IRDA
   	printk("msm_hs_startup uport->line: %d!!!\n", uport->line);

	if(1 == uport->line)   
	{
		msm_irda_register_set();
		...
		ret = vreg_set_level(vreg_gp4, 3000);
		if (ret) {
...
static void msm_hs_shutdown(struct uart_port *uport)
{
	...
	#ifdef CONFIG_USE_IRDA
   	printk("msm_hs_shutdown uport->line: %d!!!\n", uport->line);
